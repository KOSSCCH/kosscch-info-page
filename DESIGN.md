...
- Trust signals: three-university collaboration, exact date/place, organizer email, judging and prize details, verified Google Form application link
- Avoid: stale or unverified application URLs, stale 2026 spring content, dense fixed-pixel poster-only layouts
...
- Spacing/layout rhythm: centered max-width containers, responsive grids, no fixed-width content blocks; fine-pointer slideshow panels use viewport-height-aware spacing so each section remains fully visible
...
- Variants and states: application CTA links to the organizer-provided Google Form
...
- Layout adaptations: grids collapse to one column; timeline and info rows stack naturally; desktop slideshow density tightens below 900px viewport height while touch layouts retain natural page flow
...
- Empty: not applicable for the current static content
- Error: not applicable for static content
- Success: not applicable for static content
- Disabled: not applicable while the verified Google Form link is active
...
- Test/screenshot expectations: run Astro build/check; verify every desktop slideshow panel has `scrollHeight <= clientHeight` at representative 1440×900 and 1366×768 viewports; use browser screenshots when a local server is started
